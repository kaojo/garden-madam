...
class ButlerDetailsPage extends StatelessWidget {
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            Expanded(child: ListView(children: _getValves(_butler, _butlerController))),
            Divider(
              thickness: 3,
            ),
          ],
  ...
  List<Container> _getValves(Butler butler, ButlerController butlerController) {
    return butler.pins != null ? butler.pins.map((pin) => _getValve(pin, butlerController)).toList() : [];
  }

  Container _getValve(Pin pin, ButlerController butlerController) {
    return Container(
        margin: EdgeInsets.fromLTRB(0, 0, 0, 20),
        child: Row(
          children: <Widget>[
            new GestureDetector(
                onTap: () => pin.toggle(butlerController),
                child: Container(
                  decoration: BoxDecoration(
                    border: Border.all(
                        width: 5,
                        color: pin.status == Status.ON ? Colors.blue : Colors
                            .grey),
            ...
            Expanded(
                child: Text(
                  pin.name != null ? pin.name : pin.valvePinNumber.toString(),
